...
class ResourcePool {
  ...
  static get INVALID_POOL_SIZE_VALUE_ERROR_MSG() {
    return 'Invalid poolSize value. Must be positive.'
  }

  static get INVALID_POOL_SIZE_TYPE_ERROR_MSG() {
    return 'Invalid poolSize value. Must be integer.'
  }

  static get INVALID_INTERVAL_VALUE_ERROR_MSG() {
    return 'Invalid interval value. Must be positive.'
  }

  static get INVALID_CALLBACK_TYPE_ERROR_MSG() {
  ...
  constructor(poolSize, interval, callback) {
    if (!utils.isPositive(poolSize)) {
      throw new Error(ResourcePool.INVALID_POOL_SIZE_VALUE_ERROR_MSG)
    } else if (!utils.isInt(poolSize)) {
      throw new Error(ResourcePool.INVALID_POOL_SIZE_TYPE_ERROR_MSG)
    } else {
      poolSizes.set(this, poolSize)
    }

    if (!utils.isPositive(interval)) {
      throw new Error(ResourcePool.INVALID_INTERVAL_VALUE_ERROR_MSG)
    ...
    if (!utils.isFunction(callback)) {
      throw new Error(ResourcePool.INVALID_CALLBACK_TYPE_ERROR_MSG)
    } else {
